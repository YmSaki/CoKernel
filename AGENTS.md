...
Changes to `compose.yaml`, authentication, WSL hardening, GPU runtime configuration, or tunnel-client configuration are security-sensitive. Document the reason and update `docs/ARCHITECTURE.md` or `docs/SECURITY.md` when the trust boundary changes.
...
5. When tunnel credentials are configured, the tunnel runtime reports ready and ChatGPT can enumerate Jupyter MCP tools.
6. Notebook edits/executions performed through MCP are visible in JupyterLab.
